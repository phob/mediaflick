...
{
    private const string BasicSeasonEpisodeRegexPattern =
        @"^(?<title>.*?)[\. ]?[s](?<season>\d{1,2})[\. ]?(e|ep)(?<episode>\d{1,2})[-]?(?<episode2>(e|ep)?\d{1,2})?.*$";
    private const string FinerTitleRegexPattern =
        @"^(?<title>.+?)(?:\s\(?(?<year>\d{4})\)?)?\s?[-\s]*$";

    [GeneratedRegex(
        BasicSeasonEpisodeRegexPattern,
