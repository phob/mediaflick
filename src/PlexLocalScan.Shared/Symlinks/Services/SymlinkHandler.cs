...
public class SymlinkHandler(ILogger<SymlinkHandler> logger, IContextService contextService)
    : ISymlinkHandler
{
    public async Task<bool> CreateSymlinksAsync(
        string sourceFile,
    ...
    )
    {
        try
        {
            ...
            (var targetPath, var targetFileName) = GetTargetPath(
                mediaInfo,
                destinationFolder,
                extension
            );
            return await CreateSymlinkWithStructureAsync(
                sourceFile,
                targetPath,
                targetFileName,
                mediaType
        ...
        catch (Exception ex)
        {
            logger.LogError(
                ex,
                "Error creating symlinks from {Source} to {Destination}",
                sourceFile,
                destinationFolder
            );
            return false;
    ...
    private async Task<bool> CreateSymlinkWithStructureAsync(
        string sourcePath,
        string targetPath,
    ...
    )
    {
        try
        {
            var emptyMediaInfo = new MediaInfo { MediaType = mediaType };
        ...
        catch (Exception ex)
        {
            logger.LogError(
                ex,
                "Failed to create symlink structure from {Source} to {Target}",
                sourcePath,
    ...
    private async Task<bool> CreateFallbackSymlinkAsync(
        string sourceFile,
        string destinationFolder,
        MediaType mediaType
    )
    ...
    private (string path, string fileName) GetTargetPath(
        MediaInfo? mediaInfo,
        string baseFolder,
        string extension
    )
    {
        ...
        catch (Exception ex)
        {
            if (mediaInfo != null)
            {
                logger.LogError(
                    ex,
                    "Error formatting path for {MediaType}: {Title}",
                    mediaInfo.MediaType,
                    mediaInfo.Title
                );
            }
