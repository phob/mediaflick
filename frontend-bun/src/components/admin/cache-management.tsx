...
export function CacheManagement() {
  ...
  const handleInvalidateMovie = async () => {
    ...
    setIsLoading(true)
    try {
      await fetchApi("/medialookup/cache", { method: "DELETE" })
      invalidateMovie(Number(tmdbId))
      toast({
        title: "Cache Cleared",
        description: `Backend currently clears all TMDb cache. Requested movie ID: ${tmdbId}`,
      })
      setTmdbId("")
    } catch {
      toast({
  ...
  const handleInvalidateTvShow = async () => {
    ...
    try {
      ...
      toast({
        title: "Cache Cleared",
        description: `Backend currently clears all TMDb cache. Requested TV ID: ${tmdbId}`,
      })
      setTmdbId("")
    } catch {
      toast({
  ...
  const handleInvalidateSearch = async () => {
    ...
    setIsLoading(true)
    try {
      await fetchApi("/medialookup/cache", {
        method: "DELETE",
      })
      invalidateSearch(searchTitle, searchMediaType)
      toast({
        title: "Cache Cleared",
        description: `Backend currently clears all TMDb cache. Requested search: ${searchTitle} (${searchMediaType})`,
      })
      setSearchTitle("")
    } catch {
      toast({
  ...
  const handleClearAllCache = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Backend and frontend media caches were cleared",
        variant: "default",
      })
    } catch {
      toast({
  ...
  const handleGetCacheStats = async () => {
    ...
    toast({
      title: "Not Available",
      description: "Cache stats endpoint is not available in backend-bun v1",
    })
  }
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Backend v1 invalidates all cache for any clear request
            </CardDescription>
          </CardHeader>
