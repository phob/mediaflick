...
export function useSignalRHandlers({
  ...
  useEffect(() => {
    ...
    const unsubscribeAdd = signalr.subscribe("file.added", 
      (file) => handleFileAdded(file, pageSize))
    const unsubscribeUpdate = signalr.subscribe("file.updated", 
      (file) => handleFileUpdated(file, pageSize, shouldIncludeFile))
    const unsubscribeRemove = signalr.subscribe("file.removed", 
