...
import { MediaSearchResult, MediaInfo, MediaType, MediaStatus, SeasonInfo, EpisodeInfo, PagedResult, ScannedFile, UpdateScannedFileRequest, ScannedFileStats } from '@/lib/api/types';
...
export const mediaApi = {
  ...
  getTvShow: (tmdbId: number) =>
    fetchApi<MediaInfo>(`/medialookup/tvshows/${tmdbId}`),
  ...
  getImageUrl: (path: string, size: string = 'w500') =>
    fetchApi<string>(`/medialookup/images/${path}?size=${size}`),
  ...
  getScannedFile: (id: number) =>
    fetchApi<ScannedFile>(`/scannedfiles/${id}`),

  getScannedFileStats: () =>
    fetchApi<ScannedFileStats>('/scannedfiles/stats'),
