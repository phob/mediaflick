...
function cloneConfig(config: ConfigurationPayload): ConfigurationPayload {
    return {
        plex: {
            ...config.plex,
            folderMappings: config.plex.folderMappings.map((mapping) => ({
                ...mapping,
            })),
        },
        tmDb: { ...config.tmDb },
            ...
            <header class="hero">
                <div class="hero-top-row">
                    ...
                    <div class="status-chips">
                        <span class={`chip ${backendOnline() ? "ok" : "warn"}`}>
                            {backendOnline()
                                ? "Backend Online"
                                : "Backend Waiting"}
                        </span>
                ...
                <p>
                    Browse titles directly, control TV episode grouping, and
                    review movie extras from related source folders.
                </p>
            </header>
    ...
    const unidentifiedQuery = useQuery(() => ({
        ...
        queryFn: async () => {
            ...
            const typeGroups = [...groupedByType.entries()]
                .sort((left, right) => {
                    const leftOrder =
                        typeOrder[left[0]] ?? Number.MAX_SAFE_INTEGER;
                    const rightOrder =
                        typeOrder[right[0]] ?? Number.MAX_SAFE_INTEGER;
            ...
            return {
                ...
                typeGroups,
                total: files.length,
                failedCount: files.filter((file) => file.status === "Failed")
                    .length,
                duplicateCount: files.filter(
                    (file) => file.status === "Duplicate",
                ).length,
                unknownTypeCount: files.filter(
                    (file) => file.mediaType === "Unknown",
                ).length,
            };
        },
    }));
            ...
            <Show when={(unidentifiedQuery.data?.total ?? 0) > 0}>
                <section class="files-section">
                    <div class="pill-stack">
                        <span class="pill">
                            Total: {unidentifiedQuery.data?.total ?? 0}
                        </span>
                        <span class="pill">
                    ...
                    <p class="section-help">
                        Includes failed detection, duplicate destination
                        conflicts, and non-media type entries.
                    </p>
    ...
    return (
        <div class="file-row missing-file-row">
            <div>
                ...
                <p class="file-path">
                    Originally aired: {formatAirDate(props.airDate)}
                </p>
            </div>
            <div class="file-meta">
...
function TvShowDetailsPage() {
        ...
        return [...seasons.entries()]
            ...
            .map(
                ([seasonNumber, files]) =>
                    [
                        seasonNumber,
                        [...files].sort(compareEpisodeFiles),
                    ] as const,
            );
    });
    ...
    const seasonCoverageBySeason = createMemo<SeasonCoverageGroup[]>(() => {
        ...
        for (const file of tvFilesQuery.data?.categorizedFiles ?? []) {
            ...
            if (
                file.episodeNumber2 &&
                file.episodeNumber2 > file.episodeNumber
            ) {
        ...
        for (const season of seasonDetails) {
            ...
            const additionalScanned = [...seasonMap.entries()]
                .filter(
                    ([episodeNumber]) => !tmdbEpisodeNumbers.has(episodeNumber),
                )
                .sort((left, right) => left[0] - right[0]);
            ...
            const episodeCountScanned = season.episodes.filter((episode) =>
                seasonMap.has(episode.episodeNumber),
            ).length;
            groups.push({
    ...
    const selectedEpisodeGroup = createMemo(() => {
        ...
        return (
            (episodeGroupsQuery.data?.groups ?? []).find(
        ...
        const seasons = seasonGroupsToRender()
            .filter(
                (group) =>
                    ...
                    group.cards.some((card) => card.kind === "file"),
            )
    ...
    const missingEpisodeCount = createMemo(() => {
        return seasonCoverageBySeason().reduce((total, group) => {
            const missingInSeason = group.cards.filter(
    ...
    const handleEpisodeGroupChange = (nextValue: string) => {
        const currentSelection =
            episodeGroupsQuery.data?.selectedEpisodeGroupId ?? "";
                    ...
                    <>
                            ...
                            <div class="pill-stack">
                                ...
                                <span class="pill">
                                    {selectedEpisodeGroup()
                                        ? "Total selected-group episodes"
                                        : "Total TMDb episodes"}
                                    : {displayedEpisodeTotal()}
                                </span>
                                <span class="pill">
                                    Seasons scanned: {scannedSeasonCount()} /{" "}
                                    {show().seasonCount ?? 0}
                                </span>
                            </div>
                        </header>
                        ...
                        >
                            <section class="inline-controls">
                                <label for="episode-group-select">
                                    Episode grouping
                                </label>
                                <select
                        ...
                        <section class="files-section">
                            ...
                            >
                                <p class="empty-state">
                                    Loading TMDb seasons to calculate missing
                                    episodes...
                                </p>
                            </Show>
                            <Show when={seasonDetailsQuery.isError}>
                                <p class="empty-state">
                                    Unable to load TMDb season breakdown.
                                    Showing scanned episodes only.
                                </p>
                            </Show>
                            <Show when={missingEpisodeCount() > 0}>
                                <p class="section-help">
                                    Highlighted cards are episodes missing from
                                    the TMDb season order:{" "}
                                    {missingEpisodeCount()}
                                </p>
                            </Show>
                            <Show
                                when={
                                    (seasonGroupsToRender().length ?? 0) === 0
                                }
                            >
                                <p class="empty-state">
                        ...
                        <section class="files-section">
                            ...
                            <p class="section-help">
                                ...
                                Useful for release-group extras or alternate
                                episode packs.
                            </p>
                            <Show when={tvFilesQuery.isLoading}>
                                <p class="empty-state">
                                    Loading uncategorized files...
                                </p>
                            </Show>
                            <Show
                                when={
            ...
            <Show when={movieQuery.data}>
                {(movie) => (
                    <>
                        ...
                        <section class="files-section">
                            <h3>
                                Related files in the same source directories
                            </h3>
                            <p class="section-help">
                                These files are visible under this movie because
                                they share source folders.
                            </p>
                            <Show
                                when={
                            ...
                            >
                                <p class="empty-state">
                                    No related files detected as extras.
                                </p>
                            </Show>
                            <div class="season-list">
                                <For
...
function SettingsPage() {
    ...
    const saveMutation = useMutation(() => ({
        mutationFn: (payload: ConfigurationPayload) =>
            mediaApi.updateConfig(payload),
    ...
    const patchDraft = (
        updater: (current: ConfigurationPayload) => ConfigurationPayload,
    ) => {
        setDraft((current) => {
            if (!current) {
                return current;
    ...
    const removeFolderMapping = (index: number) => {
        patchDraft((current) => {
            ...
            next.plex.folderMappings = next.plex.folderMappings.filter(
        ...
        <section class="panel detail-panel">
            <header class="detail-header">
                <div>
                    <h2>Configuration</h2>
                    <p>
                        Edit backend settings directly and save to restart the
                        poller with the updated runtime config.
                    </p>
                </div>
                <div class="pill-stack">
                    <span class="pill">Endpoint: /api/config</span>
                    <span class="pill">
                        Status: {configQuery.isFetching ? "Refreshing" : "Idle"}
                    </span>
                </div>
            </header>
...
function NotFoundPage() {
    return (
        <section class="panel">
            <h2>Not found</h2>
            <p class="empty-state">
                This route does not exist in the Solid frontend.
            </p>
