...
public sealed record TvShowTestData(
    string FileName,
    string Title,
    int Season,
    int Episode,
...
public class TvDetectionServiceTests
{
    ...
    public async Task DetectTvShowAsyncWithValidFileNameReturnsMediaInfo(TvShowTestData testData)
    {
        ...
        _tmDbClient
            .GetTvShowAsync(testData.TmdbId)
            .Returns(Task.FromResult(new TvShow { Id = testData.TmdbId }));
        _tmDbClient
            .GetTvEpisodeAsync(testData.TmdbId, testData.Season, testData.Episode)
            .Returns(
                Task.FromResult(
                    new TvEpisode
                    {
                        Id = 1,
                        SeasonNumber = testData.Season,
                        EpisodeNumber = testData.Episode,
                    }
                )
            );
        _tmDbClient
            .GetTvShowExternalIdsAsync(testData.TmdbId)
            .Returns(Task.FromResult(new ExternalIdsTvShow { ImdbId = testData.ImdbId }));
        // Act
    ...
    public async Task DetectTvShowAsyncWithInvalidFileNameReturnsEmptyMediaInfo()
    {
        ...
        _tmDbClient
            .SearchTvShowAsync(Arg.Any<string>())
            .Returns(Task.FromResult(new SearchContainer<SearchTv> { Results = [] }));
    ...
    public async Task DetectTvShowAsyncWhenCachedReturnsCachedResult()
    {
        ...
        _cache
            .TryGetValue(Arg.Any<string>(), out MediaInfo? _)
            .Returns(x =>
            {
                x[1] = cachedMediaInfo;
                return true;
    ...
    [Theory]
    [MemberData(nameof(TvShowTestData.ValidTvShows), MemberType = typeof(TvShowTestData))]
    public async Task DetectTvShowByTmdbIdAsyncWithValidIdReturnsCorrectInfo(
        TvShowTestData testData
    )
    {
        // Arrange
        ...
        var externalIds = new ExternalIdsTvShow { ImdbId = testData.ImdbId };
        ...
        _tmDbClient
            .GetTvShowExternalIdsAsync(testData.TmdbId)
            .Returns(Task.FromResult(externalIds));
    ...
    public async Task DetectTvShowByTmdbIdAsyncWithInvalidIdReturnsEmptyMediaInfo()
    {
        ...
        var invalidTmdbId = -1;
        _tmDbClient
            .GetTvShowAsync(invalidTmdbId)
            .Returns(Task.FromException<TvShow>(new HttpRequestException("Invalid TMDb ID")));
